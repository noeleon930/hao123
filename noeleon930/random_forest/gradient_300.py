...
def f(i, train_data, truth_data):
	clf_gr = GradientBoostingClassifier(n_estimators=1000, learning_rate=0.001, max_depth=5, min_samples_split=i, verbose=False)
	scores = cross_val_score(clf_gr, train_data, truth_data, n_jobs=1, cv=2, verbose=False)
	print "1000 es," + str(i) + " min_samples_split:" + str(scores.mean())
...
if __name__ == '__main__':
	...
	i = 300
	clf_gr = GradientBoostingClassifier(n_estimators=200000, learning_rate=0.0001, max_depth=5, min_samples_split=i, verbose=False)
	clf_gr.fit(train_data, truth_data)
	predicts = clf_gr.predict_proba(test_data)
	
	print "Writing to csv file..."
	predicts_output = open("gr_186d_feature_selected_200000_00001_5_300.csv", 'w')
